...
- Las **7 piezas estándar** (I, O, T, S, Z, J, L) con colores diferenciados, más la **Tuerca** (3×3 con hueco central) como pieza especial de reto.
...
- **Piezas**: definidas como matrices cuadradas. Para rotar se calcula la transposición + reverso de filas (`rotateCW`). La **Tuerca** (tipo `NUT`, índice 8) es una matriz 3×3 con las 8 celdas del borde ocupadas y el centro en `0`: ese hueco es real dentro del modelo de colisiones, así que ninguna otra pieza puede "meterse" en él, y una fila que quede con ese hueco expuesto no se puede limpiar hasta romper el anillo por arriba.
